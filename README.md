...
dsh 0.1.5 出厂自带的档位只有 `web` / `acp` / `headless` / `sdk` 四个。
`dsh --help` 的示例里会出现 `--profile tui`，但那份帮助**原文本身就写了限定条件**
（`assuming the tui profile is installed`）—— 官方出货的 bundle 清单里**没有 tui**，
实测 `dsh --profile tui` 会报 `profile "tui" does not exist`。
即上游**预留了 tui 档位接口但未出货**，需要时可自行用
`dsh plugin --profile tui add <包名>` 装第三方实现。
（注：npm 上的 `@deepseek-ai/dsh-terminal` **不是**终端界面，那是给 agent 驱动 PTY 会话用的工具，
与「给人用的终端 UI」是两回事。）
...
├── config/
│   ├── launch.conf            # 默认运行模式（web / cli），命令行直启时使用
