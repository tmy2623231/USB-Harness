...
[CmdletBinding()]
param(
    [string]$Action = ''   # web=直接启动 Web 界面；cli=直接启动命令模式（dsh --profile headless）；
                           # setup=重新配置；reset=重置；status=查看状态；
                           # 空=交互菜单（按 config/launch.conf 的模式启动）
)
...
# 用户「退出」标记：launch.bat 的 Ctrl+C 重进循环靠它区分「主动退出」与「被中断」
$LauncherExitFlag = Join-Path $Root '.cache\launcher-exit.flag'
...
# 标记「用户是主动退出菜单的」，好让 launch.bat 的 Ctrl+C 重进循环真正结束。
# 每次进入菜单时先清掉这个标记，避免上一轮遗留导致下次一中断就直接退出。
function Set-LauncherExitFlag {
    try {
        New-Item -ItemType Directory -Force -Path (Split-Path -Parent $LauncherExitFlag) | Out-Null
        [IO.File]::WriteAllText($LauncherExitFlag, "exit`r`n")
    } catch { }
}

function Clear-LauncherExitFlag {
    try { Remove-Item $LauncherExitFlag -Force -ErrorAction SilentlyContinue } catch { }
}
...
function Get-LaunchModeLabel {
    param([string]$Mode)
    if ($Mode -eq 'cli') { return '命令模式（dsh --profile headless）' }
    return 'Web（图形界面）'
}
...
function Show-Status {
    ...
    if (Test-Ready) {
        ...
        if ($mode -eq 'web') {
            ...
            Write-Host '  监听地址  : 不适用（命令模式不监听端口）' -ForegroundColor DarkGray
        }
...
function Start-Web {
    ...
    Write-Step '启动 Web 界面'
    Write-Host '  正在启动服务，请稍候…' -ForegroundColor DarkGray
    Write-Host '  启动服务后,会显示url(ctrl+鼠标左键 打开网页)' -ForegroundColor DarkGray
    Write-Host '  （web: 开头、带 token 的那行才是可点击的完整地址）' -ForegroundColor DarkGray
    Write-Host "  LAN: http://<本机IP>:$usePort" -ForegroundColor Green
    ...
    # 并且这里也**不再代为打开浏览器**——地址直接打印在控制台，由用户自行 Ctrl+左键打开。
...
# 上游把入参叫 task，但这是它**从命令行角度**的称呼；对本启动器来说它就是「命令模式」——
# 用户在菜单里输入的是一条命令，dsh 执行完即退出、不进入常驻会话。故界面统一用「命令模式」，
# 同时显式给出等价的原生命令，方便用户脱离菜单直接调用 dsh（也是本模式最可靠的验证方式）。
#
# 与 Start-Web 共用同一份环境变量（DSH_HOME / PATH），模型配置与会话数据完全一致。
function Start-Cli {
    Write-Step '启动 dsh 命令模式'
    Write-Host '  说明: 这是 dsh 的命令模式（--profile headless）——输入一条命令，' -ForegroundColor DarkGray
    Write-Host '        dsh 执行完打印最终答案后退出；不是常驻交互会话' -ForegroundColor DarkGray
    Write-Host '  等价命令: dsh --profile headless "<命令>"' -ForegroundColor DarkGray
    ...
    Write-Host '请输入要执行的命令（直接回车取消）:' -ForegroundColor Cyan
    $task = Read-Host 'command'
    if (-not $task -or -not $task.Trim()) {
        Write-Host '  已取消，未执行任何命令。' -ForegroundColor DarkGray
        Start-Sleep -Milliseconds 800
        return
    }

    Write-Host ''
    Write-Host "  [command] $task" -ForegroundColor DarkGray
    Write-Host ''
...
# 交互菜单
Clear-LauncherExitFlag   # 进入菜单 = 这一轮不算「主动退出」
while ($true) {
    Show-Status
    Write-Host '  [1] 启动 Web 界面（图形化，浏览器访问）' -ForegroundColor White
    Write-Host '  [2] 命令模式（dsh --profile headless，执行完即退出）' -ForegroundColor White
